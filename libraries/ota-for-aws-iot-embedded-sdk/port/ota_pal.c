...
OtaPalStatus_t otaPal_CloseFile(OtaFileContext_t *const pFileContext)
{
    ...
    else
    {
        ...
        else
        {
            ...
            (void)otaPal_SetPlatformImageState(pFileContext, OtaImageStateTesting);
        }
    }
...
{
    (void)pFileContext;
    LogInfo(("************ --> Resetting device for new firmware...."));
    /* Short delay for debug log output before reset. */
    vTaskDelay(pdMS_TO_TICKS(15000U)); // OTA_HALF_SECOND_DELAY);
    esp_restart();
    return OTA_PAL_COMBINE_ERR(OtaPalSuccess, 0);
}
